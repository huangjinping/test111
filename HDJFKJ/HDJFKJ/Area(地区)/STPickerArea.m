...
/** 9.1省份编码 */
@property (nonatomic, strong, nullable)NSString *provinceCode;

/** 10.城市 */
@property (nonatomic, strong, nullable)NSString *city;

/** 10.1城市编码 */
@property (nonatomic, strong, nullable)NSString *cityCode;

/** 11.地区 */
@property (nonatomic, strong, nullable)NSString *area;

/** 11.1 地区编码 */
@property (nonatomic, strong, nullable)NSString *areaCode;
...
- (void)loadData
{
    ...
    NSString *path = [[NSBundle mainBundle] pathForResource:@"PCAFile.plist" ofType:nil];
    
    NSDictionary *dict = [NSDictionary dictionaryWithContentsOfFile:path];
    
    NSString * pcaString = dict[@"areaRoot"];

    self.arrayProvince = [HDProvinceModel mj_objectArrayWithKeyValuesArray:pcaString];
    
    if (self.arrayProvince.count > 0) {
        HDProvinceModel * provinceModel = self.arrayProvince[0];
        
        self.province = provinceModel.areaName;
        self.provinceCode = provinceModel.areaId;

        
        self.arrayCity = provinceModel.cities;
        
        if (self.arrayCity.count > 0) {
            HDCityModel * cityModel = self.arrayCity[0];
            
            self.city = cityModel.areaName;
            self.cityCode = cityModel.areaId;
            
            self.arrayArea = cityModel.counties;
            
            if (self.arrayArea.count > 0) {
                
                HDAreaModel * areaModel = self.arrayArea[0];
                self.area = areaModel.areaName;
                self.areaCode = areaModel.areaId;
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component
{
    if (component == 0) {
        ...
        HDProvinceModel * provinceModel = self.arrayProvince[row];
...
//        self.arrayArea = [NSMutableArray arrayWithArray:[self.arraySelected firstObject][@"areas"]];
        ...
        HDCityModel * city = self.arrayCity[row];
        
        self.arrayArea = city.counties;
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(nullable UIView *)view
{
    ...
    NSString *text;
    if (component == 0) {
        HDProvinceModel * province = self.arrayProvince[row];
        ...
        if (self.arrayArea.count > 0) {
            HDAreaModel * area = self.arrayArea[row];
            text = area.areaName;
        }else{
            text =  @"";
        }
...
- (void)selectedOk
{
    [self.delegate pickerArea:self province:self.province provinceCode:self.provinceCode city:self.city cityCode:self.cityCode area:self.area areaCode:self.areaCode];
    [self remove];
}
...
- (void)reloadData
{
    ...
    HDProvinceModel * provinceModel = self.arrayProvince[index0];
    self.province = provinceModel.areaName;
    self.provinceCode = provinceModel.areaId;
    
//    self.province = self.arrayProvince[index0];
    
    HDCityModel * cityModel = self.arrayCity[index1];
    self.city = cityModel.areaName;
    self.cityCode = cityModel.areaId;
...
//    self.city = self.arrayCity[index1];
    ...
    HDAreaModel* areaModel = self.arrayArea[index2];
    self.area = areaModel.areaName;
    self.areaCode = areaModel.areaId;
