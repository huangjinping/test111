...
- (void)clickConfirmButton:(UIButton *)sender{
    ...
    if (self.bankCardID == nil) {
        [HDLoading showFailViewWithString:@"请先选中银行卡"];
    }
    else{
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row != self.bankCardArray.count) {
        cell.rightImageView.frame = CGRectMake(LDScreenWidth - 42, 20, 20, 20);
        ...
        cell.rightImageView.frame = CGRectMake(LDScreenWidth - 26, 26, 6, 11);
        
        //NSLog(@"screenwidth = %f,,,,imageWidth = %f",LDScreenWidth,cell.rightImageView.frame.origin.x);
        cell.rightImageView.image = [UIImage imageNamed:@"返回"];
